...
def clean_transform_data(df):
    ...
    # Runs the cleaning and transformation functions
    # Stores as a new DataFrame
...
def rename_columns(df):
    ...
    # New column names
    new_cols = ['id', 'battle_time', 'arena_id', 'gamemode_id',
                ...
                'winner_card3_level', 'winner_card4_id', 'winner_card4_level',
                'winner_card5_id', 'winner_card5_level', 'winner_card6_id',
                'winner_card6_level', 'winner_card7_id', 'winner_card7_level',
                'winner_card8_id', 'winner_card8_level', 'winner_cards_list',
                ...
                'winner_elixir_average', 'loser_card1_id', 'loser_card1_level',
...
# Filters for the records that are ladder matches
# Returns this filtered DataFrame
def filter_gamemode(df):
    ...
    """
    # Filters for ladder matches (gamemode_ID = 72000006)
...
def drop_columns(df):
    ...
    Args:
        df (DataFrame): The DataFrame whose columns
    ...
    cleaned_df = df.drop('arena_id', 'winner_king_tower_hp',
                         'winner_princess_towers_hp', 'winner_clan_tag',
...
def filter_trophies(df):
    ...
    # Filters for ranked league games where the winner had >= 4k trophies
...
def keep_battle_date_only(df):
    """
     Creates a new column battle_date with only the battle date
     (removing the time).
    ...
    Returns:
        DataFrame: The DataFrame containing the column battle_date
        with just the date and with the original battle_time column dropped.
    """
    ...
    # Applies splicing function to the battle_time column
    # creates a new column "battle_date" with just the date
    cleaned_date_df = df.withColumn(
        "battle_date", first_ten_udf(df.battle_time)
        )
...
def set_types(df):
    ...
    # Set the appropriate type for all columns one by one and round floats
    df = df.withColumn("id", df.id.cast(IntegerType()))
    df = df.withColumn("gamemode_id", df.gamemode_id.cast(IntegerType()))
    df = df.withColumn(
        "avg_starting_trophies", df.avg_starting_trophies.cast(FloatType()))
    df = df.withColumn("winner_tag", df.winner_tag.cast(StringType()))
    df = df.withColumn(
        "winner_starting_trophies", df.winner_starting_trophies
        .cast(IntegerType()))
    df = df.withColumn(
        "winner_trophy_change", df.winner_trophy_change.cast(IntegerType()))
    df = df.withColumn("winner_crowns", df.winner_crowns.cast(IntegerType()))
    df = df.withColumn("loser_tag", df.loser_tag.cast(StringType()))
    df = df.withColumn(
        "loser_starting_trophies", df.loser_starting_trophies
        .cast(IntegerType()))
    df = df.withColumn(
        "loser_trophy_change", df.loser_trophy_change.cast(IntegerType()))
    df = df.withColumn("loser_crowns", df.loser_crowns.cast(IntegerType()))
    df = df.withColumn(
        "winner_card1_id", df.winner_card1_id.cast(IntegerType()))
    df = df.withColumn(
        "winner_card1_level", df.winner_card1_level.cast(IntegerType()))
    df = df.withColumn(
        "winner_card2_id", df.winner_card2_id.cast(IntegerType()))
    df = df.withColumn(
        "winner_card2_level", df.winner_card2_level.cast(IntegerType()))
    df = df.withColumn("winner_card3_id",
                       df.winner_card3_id.cast(IntegerType()))
    df = df.withColumn(
        "winner_card3_level", df.winner_card3_level.cast(IntegerType()))
    df = df.withColumn(
        "winner_card4_id", df.winner_card4_id.cast(IntegerType()))
    df = df.withColumn(
        "winner_card4_level", df.winner_card4_level.cast(IntegerType()))
    df = df.withColumn(
        "winner_card5_id", df.winner_card5_id.cast(IntegerType()))
    df = df.withColumn(
        "winner_card5_level", df.winner_card5_level.cast(IntegerType()))
    df = df.withColumn("winner_card6_id",
                       df.winner_card6_id.cast(IntegerType()))
    df = df.withColumn(
        "winner_card6_level", df.winner_card6_level.cast(IntegerType()))
    df = df.withColumn(
        "winner_card7_id", df.winner_card7_id.cast(IntegerType()))
    df = df.withColumn(
        "winner_card7_level", df.winner_card7_level.cast(IntegerType()))
    df = df.withColumn(
        "winner_card8_id", df.winner_card8_id.cast(IntegerType()))
    df = df.withColumn(
        "winner_card8_level", df.winner_card8_level.cast(IntegerType()))
    df = df.withColumn(
        "winner_total_card_level",
        df.winner_total_card_level.cast(IntegerType()))
    df = df.withColumn(
        "winner_elixir_average", df.winner_elixir_average.cast(FloatType()))
    df = df.withColumn("winner_elixir_average",
                       round(df.winner_elixir_average, 2))
    df = df.withColumn(
        "loser_card1_id", df.loser_card1_id.cast(IntegerType()))
    df = df.withColumn("loser_card1_level",
                       df.loser_card1_level.cast(IntegerType()))
    df = df.withColumn(
        "loser_card2_id", df.loser_card2_id.cast(IntegerType()))
    df = df.withColumn(
        "loser_card2_level", df.loser_card2_level.cast(IntegerType()))
    df = df.withColumn("loser_card3_id", df.loser_card3_id.cast(IntegerType()))
    df = df.withColumn(
        "loser_card3_level", df.loser_card3_level.cast(IntegerType()))
    df = df.withColumn("loser_card4_id", df.loser_card4_id.cast(IntegerType()))
    df = df.withColumn(
        "loser_card4_level", df.loser_card4_level.cast(IntegerType()))
    df = df.withColumn("loser_card5_id", df.loser_card5_id.cast(IntegerType()))
    df = df.withColumn(
        "loser_card5_level", df.loser_card5_level.cast(IntegerType()))
    df = df.withColumn("loser_card6_id", df.loser_card6_id.cast(IntegerType()))
    df = df.withColumn(
        "loser_card6_level", df.loser_card6_level.cast(IntegerType()))
    df = df.withColumn("loser_card7_id", df.loser_card7_id.cast(IntegerType()))
    df = df.withColumn(
        "loser_card7_level", df.loser_card7_level.cast(IntegerType()))
    df = df.withColumn("loser_card8_id", df.loser_card8_id.cast(IntegerType()))
    df = df.withColumn(
        "loser_card8_level", df.loser_card8_level.cast(IntegerType()))
    df = df.withColumn("loser_total_card_level", df.loser_total_card_level
                       .cast(IntegerType()))
    df = df.withColumn(
        "loser_elixir_average", df.loser_elixir_average.cast(FloatType()))
    df = df.withColumn(
        "loser_elixir_average", round(df.loser_elixir_average, 2))
