...
def add_league_column(df):
    """
     Creates a league column by taking the winner's trophies
     and calculating which league the battle occurred in.

    Args:
        df (DataFrame): The cleaned dataframe to be enriched.

    Returns:
        DataFrame: A DataFrame with a new column "league"
        containing which league the battle occurred in.
    """
    ...
    # to create a new column "league" where the battle happened
    league_df = df.withColumn(
        "league", league_udf(df.winner_starting_trophies)
        )

    return league_df


# Takes in the number of trophies that the winner has.
# Returns the appropriate league
def league_definer(trophies):
    """
     Takes the winner's trophies and
     calculates what league they were battling in.

    Args:
        trophies (int): The number of trophies of the winner.

    Returns:
        Str: A string containing the name of the league
        which the battle occurred in.
    """

    # If winner's trophies meets the league thresholds,
    # set the league to the appropriate one
    ...
    # If the winner's trophies are not >= 4000, raise an Error
    else:
